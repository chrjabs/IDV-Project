...
class AllNoneChecklist(html.Div):
    ...
    def __init__(self, options, value, max_height=None):
        ...
        html.Div.__init__(self, children=[
            dbc.Row([
                    dbc.Col(dbc.Button('All', outline=False, color='primary',
                                       id={'type': self.all_button_type, 'index': self.idx}), width=6),
                    dbc.Col(dbc.Button('None', outline=True, color='primary',
                                       id={'type': self.none_button_type, 'index': self.idx}), width=6),
                    ]),
            html.Div(dcc.Checklist(options=options, value=value,
                                   id={'type': self.checklist_type, 'index': self.idx}),
                     style=style)
        ], id={'type': 'anc', 'index': self.idx})


_callback_inputs = [
    Input({'type': AllNoneChecklist.all_button_type, 'index': MATCH}, 'n_clicks'),
    Input({'type': AllNoneChecklist.none_button_type, 'index': MATCH}, 'n_clicks')
]
_callback_output = Output(
    {'type': AllNoneChecklist.checklist_type, 'index': MATCH}, 'value')
